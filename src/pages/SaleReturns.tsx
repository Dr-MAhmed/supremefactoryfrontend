...
import { generateSaleReturnPDF, downloadPDF, openPDFForPrint } from '../lib/pdfGenerator';
...
type SaleReturnFormValues = z.infer<typeof saleReturnSchema>;
...
export default function SaleReturns() {
  ...
  const fetchReturns = async () => {
    ...
    try {
      ...
      handleError(error, 'Failed to load sale returns');
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchParties = async () => {
    try {
      ...
      setParties(data.filter((p: Party) => p.type === 'CUSTOMER' || p.type === 'BOTH'));
    } catch (error: any) {
  ...
  const editReturn = (saleReturn: SaleReturn) => {
    setEditingReturn(saleReturn);
    setValue('partyId', saleReturn.party.id);
    setValue('saleId', saleReturn.sale?.id || '');
    setValue('items', saleReturn.items.map(item => ({
    ...
    setValue('discount', saleReturn.discount);
    setValue('tax', saleReturn.tax);
    setValue('reason', saleReturn.reason || '');
    setShowForm(true);
  };
  ...
  const onSubmit = async (values: SaleReturnFormValues) => {
    ...
    try {
      ...
      if (editingReturn) {
        await api.put(`/sale-returns/${editingReturn.id}`, payload);
        showToast('Sale return updated successfully', 'success');
      } else {
        await api.post('/sale-returns', payload);
        showToast('Sale return created successfully', 'success');
      }
      reset();
      setEditingReturn(null);
      setShowForm(false);
      ...
      handleError(error, `Failed to ${editingReturn ? 'update' : 'create'} sale return`);
    } finally {
  ...
  const handlePDFDownload = useCallback((r: SaleReturn) => {
    const doc = generateSaleReturnPDF({
      voucherNo: r.voucherNo,
      date: r.date,
      party: r.party.name,
      referenceNo: r.sale?.invoiceNo,
      subtotal: r.subtotal,
      discount: r.discount,
      tax: r.tax,
      total: r.total,
    ...
    downloadPDF(doc, `Sale_Return_${r.voucherNo}.pdf`);
  }, []);

  const handlePDFPrint = useCallback((r: SaleReturn) => {
    const doc = generateSaleReturnPDF({
      voucherNo: r.voucherNo,
      date: r.date,
      party: r.party.name,
    ...
    openPDFForPrint(doc, `Sale Return #${r.voucherNo}`);
  }, []);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canEdit ? (
          ...
          <ViewOnlyNotice entity="sale returns" />
        )}
      </div>
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-700 dark:bg-slate-800/90 dark:shadow-dark-card">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                <select {...register('saleId')} className="w-full rounded-2xl border border-slate-200 px-4 py-2 text-sm">
